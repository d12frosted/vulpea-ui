...
Expand one to see its invalid notes. Each row has a =fix= button that
prompts to fix the note's violations in place, then re-indexes and
refreshes so the note drops off once it is healthy, and a link that jumps
to the note for a closer look. =g= rescans, =q= buries the buffer. The
scan is one query plus an in-memory pass, so it stays fast on a large
collection. Needs a vulpea with =vulpea-schema-collection-health= (the
=fix= button additionally needs =vulpea-schema-fix-violation=).
