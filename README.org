...
is treated as a comma-separated list of tags, so typing =wine, rated=
opens an ad-hoc view of notes carrying both tags. Empty input opens
the whole collection. Press =?= inside the view for a menu of
everything it can do; the mode line shows the note count, how many
are marked, and the active filter.
...
Columns: =title=, =tags=, =(meta KEY)=, =created=, =modified=, =links=
(outgoing link count), =backlinks= (incoming link count, one grouped
query for the whole table) and =file=. Any column takes =:name= and
=:width= overrides, e.g. =(title :width 60)=.
...
|-------+---------------------------------------------------|
| =RET= | Visit the note                                    |
| =m=   | Mark the note at point                            |
| =u=   | Unmark the note at point                          |
| =U=   | Unmark all                                        |
| =t=   | Invert marks                                      |
| =+=   | Add a tag to the selection                        |
| =-=   | Remove a tag from the selection                   |
| =e=   | Set a metadata field on the selection             |
...
nothing is marked. Marks are keyed by note id, so they survive
sorting and re-querying. Bulk tag and metadata edits go through
vulpea's batch operations (=vulpea-tags-batch-add= and friends), so
they need a vulpea recent enough to provide them. Deleting removes
the files of file-level notes; heading-level notes are skipped.

Like the sidebar, the view refreshes itself when vulpea's background
extraction lands changes in the database (vulpea 2.6+), coalescing
bulk syncs into a single refresh.
...
so the suggestion stays listed (and point stays put) until you refresh
with =g= or save - which makes it easy to link several in a row. Which
