...
note conforms, or the list of violations otherwise. Click a field to
jump to it in the note; quick-fix editing actions are tracked separately.
Needs a vulpea with the schema validation engine.
...
=-warning-face= / =-field-face= / =-message-face= inherit =success= /
=error= / =warning= / =bold= / =shadow=, so they follow your theme.
...
amber for value problems), the field, and a terse reason. Click a field
to jump to it in the note. Shown only when a schema applies.
